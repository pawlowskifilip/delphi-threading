  ...
  try
    ...
    try
      ...
      try
        ...
        Writeln(Format('Scenariusz A znalazl %d bledów.', [ListaBledow.Count]));

        ListaBledow := Walidator.WeryfikujScenariuszB(Dokument, 10);
        Writeln(Format('Scenariusz B znalazl %d bledów.', [ListaBledow.Count]));

        ListaBledow := Walidator.WeryfikujScenariuszC(Dokument, 10);
        Writeln(Format('Scenariusz C znalazl %d bledów.', [ListaBledow.Count]));
